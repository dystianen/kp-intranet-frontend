...
export class ScheduleService {
  ...
  getSchedules(): Observable<any[]> {
    return this._httpClient.get<any[]>(`${environment.apiPtnUrl}/admin/tryout-schedule`).pipe(
      map((response: any) => {
  ...
  getSchedule(id: number): Observable<any> {
    return this._httpClient.get<any>(`${environment.apiPtnUrl}/admin/tryout-schedule/${id}`).pipe(
      map((response: any) => {
        if (response.statusCode == 200) {
  ...
  createSchedule(dataUser: any): Observable<any> {
    return this.tryoutSchedules$.pipe(
      take(1),
      switchMap(sites => this._httpClient.post<any>(`${environment.apiPtnUrl}/admin/tryout-schedule`, dataUser)
  ...
  updateSchedule(id: number, data: any): Observable<any> {
    return this.tryoutSchedule$.pipe(
      take(1),
      switchMap(sites => this._httpClient.patch<any>(`${environment.apiPtnUrl}/admin/tryout-schedule/${id}`, data)
        .pipe(map((response: any) => {
          if (response.statusCode == 200) {
            return response.data;
          }
