...
export class SoalService {
    ...
    _keys: BehaviorSubject<string[]> = new BehaviorSubject([
        'A',
        'B',
        'C',
        'D',
    ...
    getSoal(id: number): Observable<any> {
        return this._httpClient
            .get<any>(`${environment.apiPtnUrl}/admin/soal/detail/${id}`)
            .pipe(
                map((response: any) => {
                    if (response.statusCode == 200) {
    ...
    createSoal(dataUser: any): Observable<any> {
        return this.soals$.pipe(
            ...
            switchMap((sites) =>
                this._httpClient
                    .post<any>(`${environment.apiPtnUrl}/admin/soal`, dataUser)
                    .pipe(
                        map((response: any) => {
                            if (response.statusCode == 200) {
                                return response.data;
    ...
    updateSoal(id: number, data: any): Observable<any> {
        return this.soal$.pipe(
            ...
            switchMap((sites) =>
                this._httpClient
                    .patch<any>(
                        `${environment.apiPtnUrl}/admin/soal/${id}`,
                        data
                    )
                    .pipe(
                        map((response: any) => {
    ...
    uploadFile(data: any) {
        return this._httpClient
            .post<any>(`${environment.apiPtnUrl}/admin/soal/upload-media`, data).toPromise();
    }
}
